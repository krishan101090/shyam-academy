...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    description:
      "Best entrance preparation classes in West Delhi for CUET, IPMAT, CLAT, NDA, SSC foundation, banking foundation, BBA/B.Com entrance and interview skills.",
    keywords: [
      "Entrance Coaching After 12th in Delhi",
      "CUET Coaching in West Delhi",
...
export default async function EntranceAfter12thPage({ params }: PageProps) {
  ...
  const breadcrumbItems = [{ label: "Home", href: "/" }, { label: "Entrance After 12th" }];

  const jsonLd = {
    "@context": "https://schema.org",
    "@graph": [
      breadcrumbListSchema(activeLocale, siteUrl, breadcrumbItems, "/entrance-after-12th"),
      {
        "@type": "Course",
      ...
      },
      {
        ...
        mainEntity: [
          ...
          {
            "@type": "Question",
            name: "Do you provide entrance coaching support in West Delhi?",
            acceptedAnswer: { "@type": "Answer", text: "Yes, students across West Delhi can request coaching and callback support through our enquiry form at our West Sagarpur centre." },
          },
        ],
      },
    ],
    ...
    <>
      ...
      <article className="mx-auto max-w-6xl px-4 py-12 sm:px-6 lg:px-8">
        ...
        <div className="mt-6 flex flex-col gap-3 sm:flex-row">
          <TrackedLocaleLink
            locale={activeLocale}
            href={contactHref("entrance-after-12th")}
            eventName="cta_click"
        ...
        <section className="mt-10 rounded-2xl border border-slate-200 bg-slate-50 p-6 dark:border-slate-800 dark:bg-slate-900/40">
          ...
          <ul className="mt-4 space-y-2 text-sm text-slate-700 dark:text-slate-300">
            ...
            <li>• CUET Coaching in West Delhi</li>
        ...
        <section className="mt-10 rounded-2xl border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900">
          ...
          <div className="mt-4 flex flex-wrap gap-3 text-sm">
            ...
            <TrackedLocaleLink locale={activeLocale} href="/subjects/accounts" eventName="cta_click" eventParams={{ cta_name: "internal_accounts_classes" }} className="rounded-full border border-slate-200 px-3 py-1 font-medium text-slate-700 hover:border-brand-300 hover:text-brand-700 dark:border-slate-700 dark:text-slate-300">
              Accounts Classes (Ajay Sir)
            </TrackedLocaleLink>
            <TrackedLocaleLink locale={activeLocale} href="/subjects/economics" eventName="cta_click" eventParams={{ cta_name: "internal_economics" }} className="rounded-full border border-slate-200 px-3 py-1 font-medium text-slate-700 hover:border-brand-300 hover:text-brand-700 dark:border-slate-700 dark:text-slate-300">
              Economics Tuition
            </TrackedLocaleLink>
          </div>
        </section>
